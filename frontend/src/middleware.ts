...
export function middleware(request: NextRequest) {
  ...
  if (request.nextUrl.pathname.startsWith('/api/')) {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://nick-the-great-api.onrender.com';
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      // Strip /api from the path when forwarding to the backend
      const path = request.nextUrl.pathname.replace(/^\/api/, '');
      const newUrl = `${apiUrl}${path}${request.nextUrl.search}`;
